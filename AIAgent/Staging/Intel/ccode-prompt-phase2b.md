...
**6a.** In the Quick Tool Commands section, after the n8n block (the one ending with "Webhook paths are per-workflow."), add:
...
**6b.** Update the skill reference line. Find:
```
...
Replace with:
```
For detailed skill instructions, read: `skills/searxng/SKILL.md`, `skills/cxdb/SKILL.md`, `skills/ollama/SKILL.md`, `skills/n8n/SKILL.md`, `skills/vision/SKILL.md`
```
...
**6d.** Update the version. Change version at top from `0.8.1` to `0.9.0` and update the version history table:
```
| 0.9.0 | 2026-03-02 | Phase 2B: Custom sandbox image (zuberi-sandbox:latest) configured. Vision skill added with Quick Tool Commands. 5 skills total. |
```
...
Update the version history with a new row:
```
| 0.8.1 | 2026-03-02 | Phase 2B: Custom sandbox image documented (zuberi-sandbox:latest). Vision skill noted. |
```

And bump the version at the top from `0.8.0` to `0.8.1`.
