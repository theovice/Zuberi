...
**Session:** 14
**Date:** 2026-03-08
**Status:** No open P0s. Zuberi is working. App at v0.1.2. RTL-051 (model upgrade) designed but NOT YET EXECUTED.
...
## Pending: RTL-051 Model Stack Upgrade

James has decided on a new model stack. The prompt is written and ready for researcher review or ccode execution.

### New stack (replacing all qwen3 models)

| Model | Ollama name | Role | VRAM | Context |
|-------|------------|------|------|---------|
| Gemma 3 12B | gemma3:12b | Primary general + vision | ~9GB | 128K |
| GPT-OSS 20B | gpt-oss:20b | Heavy reasoning / tools | ~13GB | 128K |
| Qwen2.5 Coder 14B | qwen2.5-coder:14b | Dedicated coding | ~10GB | 128K |

### Why this change
- Old stack was chosen for RTX 3060 constraints; KILO has RTX 5070 Ti 16GB
- qwen3:14b-fast had Chinese text leakage issues
- Gemma 3 12B provides text + vision in one model (replaces both qwen3 text and qwen3-vl vision)
- Dedicated coding model (qwen2.5-coder) instead of general-purpose fallback
- All three models have 128K context (up from 32K)

### Critical: API mode switch required
- Current: `baseUrl: "http://host.docker.internal:11434/v1"`, `api: "openai-completions"`
- New: `baseUrl: "http://host.docker.internal:11434"`, `api: "ollama"`
- Reason: /v1 mode breaks tool calling — models return raw JSON as text instead of structured tool calls
- Deep research and OpenClaw docs both confirm native Ollama API is required for reliable tool use
- This was deferred as RTL-044; now being done as part of RTL-051

### Compaction impact
- Old flush trigger: 32768 - 4000 - 2000 = 26,768 tokens
- New flush trigger: 128000 - 4000 - 2000 = 122,000 tokens (effectively never fires)
- Current compaction settings should work fine — no changes needed

### Models to remove from config (keep pulled in Ollama for now)
- qwen3:14b-fast
- qwen3:14b
- qwen3-vl:8b-fast
...
|----|------|--------|-------|
| RTL-051 | Model stack upgrade + native API | 🔄 | Prompt written, NOT YET EXECUTED. Models not pulled, config not changed. |
...
| RTL-050 | Capability awareness backfill | CXDB records — sent to ccode by Agent 14, verify completion |

### P3 — Future
RTL-014 (MISSION-AEGIS), RTL-016 (self-learning), RTL-018 (multi-agent), RTL-019 (gate enforcement), RTL-030 (SMS), RTL-031 (Paperclip), RTL-033 (Hugging Face), RTL-044 (absorbed into RTL-051), RTL-045c (full UI automation)

### Completed
**Architect 13 session:** RTL-042a–d, RTL-043, RTL-034, RTL-045, RTL-046, RTL-047 Phase 1, RTL-048, RTL-049
**Agent 14 session:** RTL-052 (port-127 .env fix)
...
### Models — Current (being replaced by RTL-051)
...
### Models — Target (RTL-051)
| Model | Role | reasoning | contextWindow |
|-------|------|-----------|---------------|
| gemma3:12b | Primary general + vision | false | 131072 |
| gpt-oss:20b | Heavy reasoning / tools | false | 131072 |
...
| API mode | openai-completions via /v1 (CHANGING to native ollama in RTL-051) |
...
| NOTE | This Modelfile becomes irrelevant after RTL-051 (no more qwen3 models) |
...
| Model catalog | Explicit, not auto-discovery | Too many variables at once |
...
| New model stack | Gemma 3 12B + GPT-OSS 20B + Qwen2.5-Coder 14B | RTX 5070 Ti optimized, clear roles, no overlap |
| API mode | Switch to native ollama (no /v1) | Required for tool calling reliability |
| Drop qwen3 | Chinese text leakage, 3060-era constraints | Gemma 3 is better fit for primary |
...
47. **Ollama /v1 breaks tool calling** — OpenClaw + Ollama must use native API (no /v1) for reliable tool use. OpenAI-compatible mode causes models to return raw tool JSON as text.

---

## What To Do Next

1. **RTL-051: Execute model stack upgrade** — prompt is written, ready for researcher review or direct execution
2. RTL-047 Phase 2: ToolApprovalCard UI for pending approvals
3. RTL-002: n8n bidirectional integration (Agent 14 started this)
4. RTL-050: Capability awareness backfill to CXDB
5. Zuberi is working — keep talking to her about real things

---

*Architect 13 signing off. Delay fixed, 4 models configured, one-click update, permission selector, markdown rendering, UI polish. 146 tests. Model upgrade ready. Zuberi is working.*
