...
3. Checkpoint any in-progress work to `/workspace/working/`
...
1. Save the intended write to `/workspace/working/cxdb-pending.md` (content, context ID, type)
...
2. If unhealthy, report to James — do not attempt SSH fallback without confirming
3. If healthy but task failed, report the JSON error from the dispatch response
4. Save the task prompt for retry: `/workspace/working/dispatch-retry.md`
