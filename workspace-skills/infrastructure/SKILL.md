---
name: infrastructure
description: "Hardware inventory, network topology, Docker services, port assignments, volumes, and Tailscale configuration. Use when asked about nodes (KILO, CEG), what services run where, port numbers, volume mounts, IP addresses, or system architecture. Also activates for connectivity troubleshooting: 'is the server running,' 'what port is X on,' 'how are the nodes connected,' or checking service availability. NOT for operational how-to like restart procedures (use stack-guidance)."
---
